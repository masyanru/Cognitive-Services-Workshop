...
from azure.cognitiveservices.vision.contentmoderator.models import (Evaluate, OCR, FoundFaces,  # image moderation
                                                                    APIErrorException, Body, ImageList, ImageIds, Image,
                                                                    RefreshIndex, MatchResponse,  # image lists
...
SUBSCRIPTION_KEY = ''
ENDPOINT = 'https://contentmoderatorworskhop.cognitiveservices.azure.com'
# </authentication>

# Get region for the human reviews callback endpoint
REGION = os.environ.get('CONTENT_MODERATOR_REGION', 'westeurope')
...
with open(TEXT_MODERATION_FILE, "rb") as text_file:
    screen = client.text_moderation.screen_text(language="eng", text_content_type="text/plain", text_content=text_file,
...
custom_image_list = client.list_management_image_lists.create(content_type='application/json',
                                                              body={'name': 'My Custom Images List Name',
                                                                    'description': 'A list of sport and swimsuit images',
                                                                    'metadata': {'good': 'Acceptable',
                                                                                 'not_good': 'Potentially racy'}})
...
# Returns an ImageList object
updated_list = client.list_management_image_lists.update(list_id=image_list_id, content_type='application/json',
...
for image_url in IMAGES_TO_MATCH:
    ...
    match_result = client.image_moderation.match_url_input(content_type='application/json',
                                                           list_id=image_list_id, data_representation='URL',
                                                           value=image_url)
...
custom_terms_list = client.list_management_term_lists.create(content_type='application/json',
                                                             body={'name': 'My Custom Terms List Name',
                                                                   'description': 'A list of custom terms.'})
